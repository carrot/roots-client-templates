...
class ClientCompile
  constructor: (roots, opts) ->
    ...
    {@extract, @concat, @category, @name, @out, @compress, @base} = opts
    if not @base?
      throw new Error('you must provide a base template path')

    @base = path.normalize(@base)
    @pattern = path.join(@base, opts.pattern)
    @templates = {}
  ...
  compile_hooks: ->
    before_pass: @before_hook()
    after_file: @after_hook()
    write: @write_hook()
  ...
  before_hook: (ctx) ->
    if ctx.index == ctx.file.adapters.length
      ...
      # we client-compile the file by ourselves anyway, so letting roots
      # compile it would be useless since we either replace ctx.content if not
      # @concat or we don't write the file at all otherwise
      ctx.content = ''

  ###*
   * [after_hook description]
   * @param {[type]} ctx [description]
   * @return {[type]} [description]
   * @private
  ###
  after_hook: (ctx) ->
    ...
    adapter.compileClient(ctx.original_content).then (out) =>
      ...
      tpl_name = tpl_name.split(@base)[1]
